...
- **`Game::deyilv` trait 简化签名**：`Result<f32>` → `f32`，与 `SupportCard::calc_training_effect` 同风格——deyilv 函数体内无 fail 路径（`person_index >= 6` 返回 0.0；底层 `calc_training_effect` 已去掉 Result）。`base/basic.rs`、`onsen/game.rs`、`ramen/game.rs` 三处 impl override + `ramen/game.rs` 5 处 test caller + `traits.rs::distribute_person` 一处内部 caller 全部去掉 `?`。**`Result<ActionValue, Error>::branch` 1.72% pprof 这条线又少几处分支预测开销**（deyilv 调用频次 × 每回合 5-10 次），但单点 microbench 不直接测速 deyilv，数据基本不变；与 `calc_training_value`/`calc_training_buff` 保留 `Result` 的分工清晰：前者无 fail 来源，后者带 `train >= 5` 防御性 Err（用户已决定保留）
- **`calc_training_value` 微基准 bin**：新增 `crates/umasim/tools/data_collection/calc_training_value_microbench.rs`（与 `sim_profiler` 同约定），speed build + friendship 全 100 + 固定 turn/scenario_pt 4 段（distribute_all / calc_training_buff / calc_training_value / 端到端一轮）分别测 ns/op + 占比拆分，提供 pprof 之外的"逐段基线"快速回归工具
- **`calc_training_value` microbench 扩到 7 段 + private 方法改 pub**：补完 `score_train_action`（E，单 candidate）/ `decide_train`（F，整回合打分）/ `calc_ramen_training_effect`（G，calc_value 内部嵌套拉面 buff 路径）以覆盖 d10872a microbench 与 perf_profiling pprof Top 20 的全部缺口；`RamenPolicy::score_train_action` / `status_gain`、`LocalRamenTrainer::decide_train` / `dynamic_status_adjustment` / `reserve_penalty` 由 private 提升为 pub 给性能调优工具用
- **`perf_profiling.md` 第 7 章固化按段拆解基线**：新增 §7 "按段拆解的最坏路径基线（calc_training_value_microbench）"，含 7 段新基线数据 + 与 pprof Top20 交叉校验 + 新优化优先级（distribute_all 缓存化、status_gain 优化两条新增）；附录 E 列出 `CT_MICROBENCH_RUNS` / `CT_MICROBENCH_WARMUP` env vars
