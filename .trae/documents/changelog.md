...
## 2026-08-30
- **采样地区配额**：第 2/3 年地区选择在回合末，同回合的吃面/训练决策通常先被采样器捕获——实测 1200 次自由采样 turn 23 命中 0 条、turn 47 只有 9 条。新增按工作项序号确定性分配的地区配额与「只捕获指定阶段」开关，走独立随机频道，改配额不影响其余样本；教师数据预算随之改为按配额线性计算
- **NN 落盘格式不冻结**：08-29 那句「格位表即落盘格式」应拆成两件事——234 维**动作 ID 空间**仍冻结，但「每候选 32 个 u16 分位数」的**落盘**方案否决（分数上限本就是 10 万、u16 溢出；且 mean 与 weighted_mean 本来就不在同一份数据上算），改为存原始 rollout 分、标签生成推迟到训练期。完整结论与未决项落 nn_pipeline_plan §6.5

## 2026-08-29
- **拉面 NN policy 格位表**：新增 policy_schema 模块，把动作映射到 234 维固定格位并由单一入口分派；吃面按地区 ID 而非槽位编码、吃面与万能风味用法合成联合格、地区选择纳入第一代。**格位表即落盘格式，冻结**
