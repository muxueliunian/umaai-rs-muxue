...
## 2026-08-23
- **拉面杯 MCTS 训练员**：新增 `trainer/ramen_mcts_trainer.rs`（手写策略 + 扁平搜索）。`RamenSearchStages` 按阶段门控决定哪些决策点走 `FlatSearch<RamenGame>`，其余转发内置 `RamenHandwrittenTrainer`；门控全关时与纯手写策略逐位一致。`bench_base` 加 `--trainer mcts` 及 `--search-n / --search-stages / --search-ucb / --search-selection / --radical-factor`；`main.rs` 的 `scenario="ramen" + trainer="mcts"` 接入该训练员（阶段由 `[mcts] ramen_search_stages` 配置，缺省 `train`）
- **搜索层根动作接口**：`FlatSearchGame` 新增 `apply_root_action`，由各剧本指定 rollout 根动作的执行路径（拉面走策略流、温泉走通用 `apply_action`）；`SearchOutput<A>` 新增 `best_action_pt_idx()`
- **手写策略地区打分覆盖第 1 年**：新增 `ramen_effective_stage`，按动作类型判定有效阶段，使 `run_begin` 内联触发的第 1 年地区选择也进入 `decide_region` 打分。**改变手写策略基线数值**
- **rollout 基策去除日志开销**：`RamenHandwrittenTrainer` 新增 `collect_breakdown` 开关与 `for_rollout()` 构造器，搜索的 rollout 实例不再采集分解文本（整局耗时 −15%）
- **拉面局面特征编码器（NN 管线 Phase 2 下半）**：新增 `game/ramen/features.rs`，把 `RamenGame` 编码为定长 `f32` 向量（global / cards / persons 三段，人头段按最大 13 个定长 + 已登场掩码）。相比温泉版补上成长率与上限并开启人头分支；分块记账、归一化尺度注明依据、查表失败一律报错不填 0；只喂原始状态与纯计数派生量，不喂带权重的估值。配套把 `policy::remaining_race_slots` 提升为 `pub(crate)`
- **人头下标与卡组槽位解耦**：`Person` trait 新增 `card_id()`、`Game` trait 新增 `deck_index_of()`，由人头的 `card_id` 反查卡组槽位。拉面的人头顺序与卡组顺序不一致，原先靠 `person_index < 6` 判定的调用点全部改为反查。**改变拉面模拟数值，bench 基线、sampler 根局面与旧编码器落盘的教师数据全部作废**
- **地区选择 build 自适应**：`score_region` 新增 `youqing × at_trains × 卡组 bias` 项，`xunlian` 与 `youqing` 统一按 `bias_sum` 缩放；`default_config.toml` 的 `ramen_region_strategy` 由 `fixed` 恢复 `all`
