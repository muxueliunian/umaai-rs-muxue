...
**阶段 4-6 GameView & disable_log 优化**：`output::view::GameView` 由 4 字段扩至 8 字段（新增 max_turn/max_vital/skill_pt/total_hints），`Game` trait 加默认 `view()`；删除 `disable_log`/`enable_log`（阶段 2 后 `diag!` 的 `diag` feature 已编译期裁剪规则层日志）；`cargo bloat` 验证显示 feature 关闭时 `libumasim.rlib` 减少 ~270 KB、`"diagnostic"` 字符串 19 → 6，确认"零运行时开销"目标。详见 `log_refactor_plan.md` §7.4-7.6。

**LOGGER 锁与 warning 清理**（阶段 6 收尾）：合并 `INIT_LOCK` (`Mutex<()>`) + `LOGGER_INIT_DONE` (`AtomicBool`) 为单个 `LOGGER_INIT` (`OnceLock<()>`) + `get_or_init`，三个 init 函数少 80 行样板；11 处 unused import / dead field / unused param 清理（含 `TrainParams::ramen_effect` 字段、`get_workspace_root` 移入 `cfg(test)` 模块），release 编译零项目 warning。
