...
## 2026-09-10
- **本机 Release 性能优化**：启用速度优先优化、ThinLTO 和本机 CPU 指令集，减少模拟、评分与搜索中的重复分配、复制和计算，候选内部模拟并行执行并按原序归并，保持同种子决策结果及决策 CSV 一致。
- **性能基准与验证**：基准继承游戏线程和搜索配置，补充分布重置、加权统计、训练预演、终局评分及跨线程失败槽一致性检查，记录整局与高预算搜索根的配对测量口径和复测步骤。
