...
- **拉面杯逐年观测出口**：`scenario_pt` / `eat_count` 在每年 RMJ 结算回合（23/47/71）清零，而育成结束在回合 77、72–77 不再吃面，局末读到的两个值恒为 0——results.csv 这两列 2100 局全零，`minimal_strategy_ab` 已在外部重算绕过且口径与 GameOutcome 分叉（一个是三年合计、一个写着当年）。改为在归零前按年归档 PT 与吃面次数，并新增逐年地区选择归档；CSV 换成 `scenario_pt_y1..y3` / `eat_count_y1..y3` / `region_y1..y3`，外部重算一并删除。地区归档的年份索引按回合硬编码（2/23/47 → 0/1/2），不能用 `current_year()`：回合 23 时它仍判第一年，但那一刻选的是第二年地区；该映射与阶段分发共用一处，顺带移除原有的 `unreachable!`。**纯观测出口，模拟数值逐位不变，基线不受影响**
- **第三方库引用规范化（续）**：bench 模块中 anyhow 宏的全名引用改为 use 导入后直接调用
- **分身分配由概率重试改为合法集直选**：两条分身路径（超级拉面 per-卡、地区拉面 per-训练位）原先都是「随机抽一个落点、放不下再抽」，超级侧还带 8 次重试上限——只剩一格合法时约 10% 的概率明明放得下却放弃（实测 256 种子漏放 31 次），失败仅打诊断日志、不计数不告警。改为先过滤合法落点再均匀抽取，落点条件分布不变而假失败归零；超级拉面另需友人卡优先分配，否则普通卡会先占满友人唯一的合法位（友人受「每训练一个友人」约束，落点常只剩一格）。规则上真放不下时跳过该卡、不中断育成——返回错误会作废整次搜索 rollout，且只丢弃无解局面这一类盘面。同时抽出共用的落点判定与写入，消除两条路径各写一份满员 / 挤 NPC 规则的漂移。**改变拉面模拟数值，既有基线作废**
- **分身分配随机流与策略流解耦**：分身改用按 `(rule_master, 回合, 流标记)` 派生的局部流，策略流消耗从「随重试次数在 6~48 间浮动」降为 **0**，且结果不再受本回合此前消耗次数影响。此前分配算法的任何改动都会平移同回合后续的训练成败 / 休息 / 外出随机；地区分身在吃面落地时执行，上游任一处位移都会改掉选卡。对 MCTS 尤为关键：同回合各候选动作走过不同路径后策略流长度不同，原方案会让各候选的分身随机性去相关，现方案使其拿到同一份，配对方差削减更彻底。未注入 rule_master 的旧路径回退为从父流取一个字派生，保持既有可复现性契约
