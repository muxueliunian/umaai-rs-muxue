...
- **R8 DAgger 采集配方冻结**：roll-in 换 `ens_R67mix60k_g123`（同语料三种子各训 60k 步的集成，闭环三面板均胜 30k 版集成），六份清单共 8 万有效根（gen2 四构成、双智、三个新空间、实战配置），留出与上一轮同批，号段登记在 `nn_model_registry` 第 10 节

## 2026-09-19
- **根基准新增整局路由 `hybrid-y3-full-mean`**：第三年地区选择走完整续跑但仍按均值选动作，其余根按 `--leaf-h` 截断，使其相对全局截断只改「是否截断」一件事；原有两种路由逐字不变，路由判定单测补充对应检查
