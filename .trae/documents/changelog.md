...
## 2026-09-14
- **新教师独立采集交付**：隔离分支同步上游新手写PT定价与统一MCTS接口，保留采集和探针能力；不混入地区NN接入工作树。
- **按有效根持续采集**：新增高预算通用及地区专项清单、独立序号与原留出排除，支持显式无截止、教师参数防误配、低磁盘保护和导出失败恢复；旧配方恢复语义保留。
- **资产与交接验证**：新增无哈希参考资产准备、假进程恢复测试与云端交接说明，模型旁车保持原文，实际配置与游戏基座逐字段或逐字节核对。
