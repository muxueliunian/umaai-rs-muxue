...
- **搜索掉分归因（radical_factor）**：此前对照中搜索低于纯推荐策略，原因是缺省 `radical_factor_max=50` 的加权均值有效样本量恒为 3.9%（与 search_n 无关）——等于只用 5 个样本挑最优，选择偏差压过搜索收益。显式传 `radical_factor=0` 取真均值后方向反转为显著正收益。缺省值不动（温泉在用），由调用方显式指定；具体增益曲线另行测量
