...
## 2026-08-23
- **编码器训练位改 multi-hot（分身不再从特征里消失）**：`person_train_slots` 原先用 `slots[idx] = Some(t)` 后写覆盖，只保留编号最大的训练位。拉面的分身不新建人头、而是把同一个 `person_index` 再 push 进另一个训练位（地区分身与超级拉面分身皆如此），于是彩圈分身在特征里直接消失。改为 `Vec<[bool; TRAIN_NUM]>` 掩码，cards 段与 persons 段同步逐位写标志，维度不变。`ef99478` 的 changelog 与 issues 都写了此项已完成，实际未落地，两处已订正
- **修复「人头下标当卡组下标」（上游遗留，已获上游授权）**：`Person` trait 新增 `card_id()`、`Game` trait 新增 provided 方法 `deck_index_of`，由人头的 `card_id` 在卡组里 `position()` 反查槽位，无卡人头（理事长 / 记者 / NPC）返回 `None`。拉面的 `init_persons` 只放 5 张训练卡再追加理事长、友人卡延后到回合 2 加入，于是理事长占人头 5、友人卡占人头 6，而 `deck[5]` 正是友人卡——原先靠 `person_index < 6` 判定「这个人头是卡组里的卡」的全部调用点因此串位。改为反查的有：`traits.rs` 的 `default_calc_training_buff`（核心数值路径，此前理事长顶着友人卡的完整训练加成进训练、友人卡本人加成全零）与 `distribute_hint`、拉面的 `add_friendship` / `deyilv` / `distribute_hint` override / `handle_hint_event` / `push_hint_event`。两个下标严格分开：`is_shining_at` 与 `distribution` 一律用人头下标，只有 `deck[..]` 用反查结果；`persons_mut()` 循环内改为预抽 `(card_id, hint 加成)` 再按 `card_id` 匹配以避开借用冲突；`deyilv` 的负数人头下标此前会算出 `deck[usize::MAX]` 而 panic，现安全返回 0。base / onsen 的同模式代码未动——那两个剧本 `persons[0..6)` 与 `deck` 确实同序，当前行为正确。**本修复改变拉面模拟数值，bench 基线、sampler 根局面与按旧编码器落盘的教师数据全部作废，须重跑**
- **编码器同步改为按 `card_id` 反查 + 移除恒零特征块**：`features.rs` 的 cards 段与 persons 段此前照抄了「人头下标 0..5 与卡组同序」的假设——卡槽 5 会读到理事长的训练位、人头 6 的卡链接整个丢失；剧本友人旗标用 `friend.person_index` 判定，而该字段在回合 0-1 还是卡组下标，期间会把理事长标成友人（改判 `PersonType::ScenarioCard`）。同时移除 `current_effect` 的 14 维特征块（该字段全仓库无写入点、恒为全零），`INPUT_DIM` 由 766 降至 752；**上游的 `RamenGame::current_effect` 字段本身未动**，删公开字段会扩大合并冲突面，去留留给上游
- **测试**：两个只读诊断测试改造为真回归（`test_person_deck_index_mapping_full_game` 校验友人卡两份羁绊同步、`test_training_buff_person_deck_mapping` 校验理事长 buff 全零且友人 buff 来自 `deck[5]`）；`test_ramen_deyilv_includes_scenario_bonus` 删掉写死的「`person_index >= 6` 返回 0」断言（人头 6 恰是友人卡），改为按 `PersonType` 定位理事长与友人；新增 `test_card_person_cross_lookup` 校验编码器两段的交叉反查。按项目约定，新增校验一律走 `println` 而非 `assert`，两个测试模块各加一个 `check(bool) -> "OK"/"NG"` 前缀助手，回归失效时扫输出里的 `NG` 即可定位且不中断后续诊断信息。全量 228 passed / 0 failed / 2 ignored
- **文档订正**：`ef99478` 的 changelog 与 issues 都写了「编码器侧已按 `card_id` 反查修正」「已移除 `current_effect` 的 14 维块」，与当时的代码不符，两处均已订正；另新增一条 issues 记录本次评审发现但**未纳入修复范围**的五处上游问题（训练人数加成硬编码「理事长 = 6、记者 = 7」、超级拉面分身漏掉友人卡、base / onsen hint 路径的潜伏越界、`BaseGame::new` 不校验卡组 `card_id` 重复、注释块内的旧假设），待与上游确认后另开
