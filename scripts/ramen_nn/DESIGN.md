...
这张表只是未显式传结构参数时的自动选择，**不是**当前实验配方。第四轮起的对照实验
全部显式传入 96 / 2 / 256 / 2 / 0.08；做容量对照时必须把结构参数全部写明，否则
样本数跨过 120k 会静默换成上表末行。已做过的 128 / 384（层数与 dropout 不变）
对照结果与证据边界见 `.trae/documents/nn_pipeline_plan.md` §15.2。
